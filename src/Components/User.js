...
function User({user}) {
  ...
  return (
    <section className="User flex flex-col items-center mb-4 relative">
      <div onClick={() =>dispatch(getFavoritesUser(user))} className={`" right-[37%] absolute top-12 "${choose && " text-red-600"}`}>
      ...
      <a href={user.html_url} className="py-5 hover:underline"  target="_blank" rel="noreferrer">See github profile</a>
    </section>
  );
}
